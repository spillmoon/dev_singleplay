...
function listRsv(callback) {
    var sql_select_rsvlist = 'SELECT p.name, place.placeName, p.playDay, p.playTime, p.VIPprice, p.Rprice, p.Sprice, p.salePer, p.starScoreAvg, i.imageName ' +
...
function findRsv(rsvId, callback) {
    ...
    dbPool.getConnection(function(err, dbConn) {
       ...
       dbConn.query(sql, [rsvId], function(err, result){
           ...
           rsv.poster = url.resolve('https://localhost:4433/posterimg/', path.basename(result[0].imageName));
